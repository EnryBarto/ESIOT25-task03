...
\paragraph{}
La comunicazione dei dati rilevati al CUS avviene tramite protocollo \textbf{MQTT}, pubblicando la lettura nel topic \texttt{water\_level} (spiegazione dettagliata nella \cref{sec:Mqtt}).
...
\paragraph{}
Come per il TMS, l'architettura del WCS è basata su delle \textbf{task}: l'Arduino non è però dotato di un sistema operativo, rendendo quindi necessaria l'implementazione di uno \textbf{scheduler} cooperativo, basato in questo caso sul Timer2 del microcontrollore.

Data la semplicità di questo sottosistema, è stata sviluppata una sola task basata su una \textbf{macchina a stati finiti sincrona} (FSM), di cui si riporta lo schema in \cref{fig:WcsStateDiagram} a Pag. \pageref{fig:WcsStateDiagram}.

In questo caso è importante che il \textbf{periodo della task} legato alla lettura del pulsante di cambio modalità non sia superiore al \textbf{MEST} per non perdere la pressione dello stesso.
...
\paragraph{}
Per comunicare la distanza del livello dell'acqua dalla cima della cisterna, il \textbf{TMS pubblica} la lettura del sonar a \textbf{intervalli regolari} tramite MQTT nel \textbf{topic \texttt{water\_level}}, utilizzando il broker messo a disposizione dal CUS (che \textbf{non richiede autenticazione}). Poiché è presente un solo dato da comunicare non è stato utilizzato un particolare formato di comunicazione (ad esempio JSON), ma viene inviato solo il valore letto. Nel caso di errori (ad esempio mancata lettura del sonar), il TMS pubblica il valore -1 che viene scartato dal CUS, entrando quindi in stato \texttt{UNCONNECTED} se gli errori di lettura persistono.

Per l'utilizzo del protocollo MQTT è stata utilizzata la libreria \href{https://github.com/knolleary/pubsubclient}{\texttt{PubSubClient}}. Questa libreria ha la limitazione di eseguire il \textbf{publish} solo con \textbf{QoS 0} (At Most Once): nel nostro caso non è comunque un problema grave, poiché se un messaggio dovesse andare perso basterebbe attendere il prossimo visto che vengono pubblicati con una certa frequenza.

\chapter{Test}
Sono stati realizzati una serie di test: di ognuno si riporta il link al video (in cui si visualizzano DBS, TMS e WCS) e una breve descrizione. Tutte le configurazioni utilizzate sono quelle descritte nell'\cref{chap:Config}.

\begin{itemize}
	\item \textbf{Accensione} del sistema: Inizialmente il DBS mostra \texttt{NOT\_AVAILABLE} e il TMS ha il LED rosso acceso. Quando il CUS viene messo in esecuzione, tutti i sistemi si aggiornano, partendo in modalità \texttt{AUTOMATIC}: il TMS accende il LED verde, il WCS si riavvia con il valore corretto di apertura e il DBS mostra lo stato corrente del sistema. \href{https://liveunibo-my.sharepoint.com/:v:/g/personal/enrico_bartocetti_studio_unibo_it/IQAsZnIv4J2JSoQF0lA0R8HVATj4pE9dVtz6AUaT9wTUZ4M?nav=eyJyZWZlcnJhbEluZm8iOnsicmVmZXJyYWxBcHAiOiJPbmVEcml2ZUZvckJ1c2luZXNzIiwicmVmZXJyYWxBcHBQbGF0Zm9ybSI6IldlYiIsInJlZmVycmFsTW9kZSI6InZpZXciLCJyZWZlcnJhbFZpZXciOiJNeUZpbGVzTGlua0NvcHkifX0&e=hxe27M}{Link}
	\item \textbf{Modalità automatica}: Il sistema in modalità \texttt{AUTOMATIC} imposta l'apertura della valvola basandosi sul livello di riempimento della cisterna. Quando rimangono meno di 50cm per più di 5 secondi la valvola viene aperta al 50\%, quando rimangono meno di 30cm la valvola viene immediatamente aperta al 100\%. Alla diminuzione del livello dell'acqua invece, non appena si sale sopra i 30cm la valvola viene ridotta prima al 50\% e poi chiusa completamente sopra i 50cm. \href{https://liveunibo-my.sharepoint.com/:v:/g/personal/enrico_bartocetti_studio_unibo_it/IQBuXnR9tUnrQqSlUGf5JuLFAQa9ptnqnSguED-beged36I?nav=eyJyZWZlcnJhbEluZm8iOnsicmVmZXJyYWxBcHAiOiJTdHJlYW1XZWJBcHAiLCJyZWZlcnJhbFZpZXciOiJTaGFyZURpYWxvZy1MaW5rIiwicmVmZXJyYWxBcHBQbGF0Zm9ybSI6IldlYiIsInJlZmVycmFsTW9kZSI6InZpZXcifX0%3D&e=HxxLWE}{Link}
	\item \textbf{Modalità manuale}: Il sistema quando entra in modalità \texttt{MANUAL} imposta la valvola sul valore letto dal potenziometro. Il livello di apertura della valvola può essere impostato sia tramite potenziometro nel WCS sia tramite slider del DBS. È possibile passare dalla modalità automatica alla manuale utilizzando il bottone nel DBS oppure il pulsante fisico nel WCS. Mentre il sistema è in modalità manuale il TMS continua a registrare valori, e non appena si torna in modalità \texttt{AUTOMATIC} la valvola viene impostata sul valore corretto basato sulla lettura del TMS. \href{https://liveunibo-my.sharepoint.com/:v:/g/personal/enrico_bartocetti_studio_unibo_it/IQDzOg-c6aHOToVhTKMoPKTNAXIna3SqujeVlrF9vGcsIn4?nav=eyJyZWZlcnJhbEluZm8iOnsicmVmZXJyYWxBcHAiOiJPbmVEcml2ZUZvckJ1c2luZXNzIiwicmVmZXJyYWxBcHBQbGF0Zm9ybSI6IldlYiIsInJlZmVycmFsTW9kZSI6InZpZXciLCJyZWZlcnJhbFZpZXciOiJNeUZpbGVzTGlua0NvcHkifX0&e=NryXGy}{Link}
	\item \textbf{Errore di rete}: Quando il TMS non invia dati per più di 7 secondi per un qualasi motivo (in questo caso mancata alimentazione), il sistema passa in modalità \texttt{UNCONNECTED}. Non appena la connessione viene ripristinata e il TMS comunica una nuova lettura al CUS, viene ripristinato lo stato in cui ci si trovava prima dell'errore. \href{https://liveunibo-my.sharepoint.com/:v:/g/personal/enrico_bartocetti_studio_unibo_it/IQBn3b7hFssYT6h9Sv3IDvERASubILY3hp-wZ4YG8JNJ6Ss?nav=eyJyZWZlcnJhbEluZm8iOnsicmVmZXJyYWxBcHAiOiJPbmVEcml2ZUZvckJ1c2luZXNzIiwicmVmZXJyYWxBcHBQbGF0Zm9ybSI6IldlYiIsInJlZmVycmFsTW9kZSI6InZpZXciLCJyZWZlcnJhbFZpZXciOiJNeUZpbGVzTGlua0NvcHkifX0&e=g9aUBb}{Link}
...
\begin{tabular}{lll}
\toprule
\textbf{Task} & \textbf{Periodo [ms]}\\ \midrule
Logger Task & 3000\\
Connection Task & 500\\
